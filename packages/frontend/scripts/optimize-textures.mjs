...
/** Remove anything in public/textures that isn't a `.ktx2` (e.g. macOS `.DS_Store`),
 *  so a local build can't copy stray files out of `public/` into `dist/`. */
async function pruneOutDir() {
  for (const entry of await fs.readdir(OUT_DIR)) {
    if (!entry.endsWith('.ktx2')) {
      await fs.rm(path.join(OUT_DIR, entry), { force: true });
      console.log(`  pruned stray file: textures/${entry}`);
    }
  }
}
...
async function main() {
  ...
  for (const tex of queue) {
    ...
    const outPath = path.join(OUT_DIR, `${tex.name}.ktx2`);
    ...
    try {
      ...
      await fs.writeFile(outPath, ktx2);
      totalIn += srcBuf.length;
      totalOut += ktx2.length;
      console.log(
        `✓ ${tex.name.padEnd(20)} ${mode.padEnd(11)} ${res.padEnd(11)} ` +
        `${fmtMB(srcBuf.length).padStart(9)} → ${fmtMB(ktx2.length).padStart(9)}`,
      );
    } catch (err) {
